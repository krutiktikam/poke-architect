...
from .utils import calculate_team_stats, calculate_type_coverage, suggest_pokemon
...
def analyze_team(pokemon_ids: List[int], db: Session = Depends(get_db)):
    if not pokemon_ids:
        return {"total_stats": {}, "type_coverage": {}, "suggestions": []}
    ...
    # Get all pokemon for suggestions (limited to Gen 1 for demo performance)
    all_p = db.query(Pokemon).limit(151).all()
